...
def get_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="A template for python projects.", add_help=False
    )
    # Required Args
    ...
    optional_args.add_argument(
        "-u", "--username", help="The Username of the YouTube Channel"
    )
    optional_args.add_argument(
        "--acc_mode", default="id", help="The mode of the accumulator (id or search)."
    )
    optional_args.add_argument(
        "--channel_ids_file", help="A file with a list of YouTube urls"
    )
    optional_args.add_argument(
        "--n-recent",
        default=50,
    ...
    optional_args.add_argument(
        "-d", "--debug", action="store_true", help="Enables the debug log messages"
    )
    optional_args.add_argument(
    ...
    if (args.id is None and args.username is None) and args.run_mode in [
        "add_channel",
        "remove_channel",
        "set_priority",
    ]:
        parser.error(
            "You need to pass either --id or --username when selecting "
            "the `add_channel`, `remove_channel`, or `set_priority` actions"
        )
    if (args.priority is None) and args.run_mode in ["set_priority"]:
        parser.error(
            "You need to pass --priority when selecting the `set_priority` action"
        )
    return args
...
def set_priority(youtube: YoutubeManager, args: argparse.Namespace) -> None:
    youtube.set_priority(
        channel_id=args.id, username=args.username, priority=args.priority
    )
...
def main():
    ...
    max_posted_hours = (
        ...
        if "max_posted_hours" in you_conf["config"]
        else 24
    )
