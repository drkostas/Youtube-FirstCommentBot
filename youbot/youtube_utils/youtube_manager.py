...
class YoutubeManager(YoutubeApiV3):
    ...
    def __init__(
        ...
        if cloud_conf is not None:
            ...
            self.dbox._handler = Dropbox(
                app_key=cloud_conf["app_key"],
            ...
            if self.comments_src == "dropbox":
                raise YoutubeManagerError(
                    "Requested `dropbox` comments type "
    ...
    def commenter(self):
        ...
        _, video_links_commented = self.get_comments(
            channel_ids=channel_ids, n_recent=500
        )
        commented_comments, _ = self.get_comments(
            channel_ids=channel_ids, min_likes=1, n_recent=500
        )
        ...
        while True:
            ...
            loop_cnt += 1
            if (
                loop_cnt > self.reload_data_every and sleep_time > self.fast_sleep_time
            ...
            try:
                ...
                for video in self.get_uploads(
                    channels=channel_ids, max_posted_hours=max_posted_hours
                ):
                    video_url = f'https://youtube.com/watch?v={video["id"]}'
                    if video_url not in video_links_commented:
                        comment_text = self.get_next_template_comment(
                        ...
                        if (
                            curr_loop_time
                            < delay_comment[video["channel_id"]] - sleep_time
                            and loop_cnt > 1
                            and self.api_type != "search"
                        ):
                            ch_delay = int(
                                delay_comment[video["channel_id"]]
                                - curr_loop_time
                                - sleep_time
                            )
                            logger.info(
                ...
                if "SERVICE_UNAVAILABLE" in str(e):
                    ...
                    if len(self._apis) > 1:
                        ...
                        logger.warn(f"Switching to new API ({len(self._apis)} left)..")
                        errors = 0
                        dont_sleep = True
                    else:
                        dont_sleep = False
                    ...
                    added_comment is True
                ):  # Raise fatal exception if error after commented
                ...
                if errors > 5:
                    ...
                    if sleep_time < 60:
                        sleep_time = 1800
                    logger.info(
                        "More than 5 errors! Will sleep until"
                        f" {datetime.utcnow() + timedelta(seconds=sleep_time)} ({sleep_time} seconds)"
                    )
                    self.upload_logs()
                    loop_cnt = 0
                    errors = 0
                ...
                if 4 <= datetime.utcnow().hour <= 11:
                    ...
                    sleep_time = (
                        self.fast_sleep_time
                    )  # check every second when close to new hour
                else:
                    sleep_time = self.default_sleep_time
            ...
            try:
                for (
                    ...
                    commented_comments[video["channel_id"]].append(
                        {
                            "channel_id": video["channel_id"],
                            "video_link": video_url,
    ...
    def _accumulator_cid(self):
        ...
        while True:
            ...
            exceptions = []
            try:
                for comment in self.db.get_comments(
                    comment_cols=["video_link"],
                    n_recent=self.num_comments_to_check,
                    only_valid_comment_texts=True,
                ):
                ...
                if len(exceptions) >= int(self.num_comments_to_check):
                    ...
                    logger.error(f"{traceback.format_exc()}")
                sleep_time = self.default_sleep_time
                logger.info(f"Updated {updated}/{self.num_comments_to_check} comments.")
    ...
    def _accumulator_search(self):
        ...
        while True:
            ...
            try:
                for comment in self.db.get_comments(
                    comment_cols=["video_link"], n_recent=self.num_comments_to_check
                ):
                    link = comment["video_link"]
                    if link is None:
                        continue
    ...
    def like_bot(self):
        ...
        # }
        print(req_data)
        response = requests.post(url=self.like_bot_url, data=req_data)
        print(response.text)
        return
        while True:
            try:
                for c_id, url, req_likes in self.db.get_non_ordered_comments(
                    max_minutes=self.like_bot_max_mins
                ):
                    ...
                    if "order" in response:
                        logger.info(
                            f"Ordered {req_data['quantity']} likes for:"
                            f" {req_data['link']}"
                        )
                    else:
    ...
    def list_comments(
        ...
        min_replies: int = -1,
        max_likes: int = 99999,
        max_replies: int = 99999,
        max_latency: int = 99999,
    ) -> None:
        comment_cols = [
            "comment_time",
            "upload_time",
        ...
        for row in self.db.get_comments(
            ...
            if row["upload_time"] != "-1" and row["upload_time"] != "None":
                upload_seconds_passed = int(
                    arrow.get(row["upload_time"])
                    .humanize(granularity="second")
                    .split(" ")[0]
                )
                comment_seconds_passed = int(
                    arrow.get(row["comment_time"])
                    .humanize(granularity="second")
                    .split(" ")[0]
                )
    ...
    def add_channel(
        ...
        if channel_id:
            ...
            raise YoutubeManagerError(
                "You should either pass channel id or username to add channel!"
            )
        if channel_info:
    ...
    def remove_channel(self, channel_id: str = None, username: str = None) -> None:
        if channel_id:
            ...
            raise YoutubeManagerError(
                "You should either pass channel id or username to remove channel!"
            )
    ...
    def refresh_photos(self):
        channel_ids = [
            ...
            for channel in self.db.get_channels(
                channel_cols=["channel_id"], where="TRUE"
            )
        ]
        try:
    ...
    def set_priority(
        self, channel_id: str = None, username: str = None, priority: str = None
    ) -> None:
        if channel_id:
            channel_info = self.get_channel_info_by_id(channel_id)
        elif username:
    ...
    def fill_upload_times(self, n_recent, min_likes, min_replies):
        ...
        for video in self.get_video_info(videos=video_ids):
            ...
            self.db.update_comment(
                video_link=video_link, upload_time=video["upload_time"]
            )
    ...
    def fix_comment_links(self, n_recent, min_likes, min_replies):
        video_info = [
            ...
            for row in self.db.get_comments(
                comment_cols=["video_link", "comment_id"],
                n_recent=n_recent,
                min_likes=min_likes,
                min_replies=min_replies,
            )
        ...
        for video_id, comment_id in video_info:
            ...
            self.db.update_comment(video_link=video_link, comment_id=comment_id)
    ...
    def fill_video_titles(self, n_recent, min_likes, min_replies):
        ...
        for video in self.get_video_info(videos=video_ids):
            video_link = f"https://youtube.com/watch?v={video['video_id']}"
            self.db.update_comment(
                video_link=video_link, video_title=video["video_title"]
            )

    def retrieve_old_channels(self, n_recent, min_likes, min_replies):
        commented_channel_ids = [
            comment["channel_id"]
            for comment in self.db.get_comments(
                comment_cols=["channel_id"],
                n_recent=n_recent,
                min_likes=min_likes,
                min_replies=min_replies,
            )
    ...
    def get_comments(self, n_recent, channel_ids=None, min_likes: int = -1):
        ...
        if channel_ids is None:
            commented_comments[None] = list(
                self.db.get_comments(
                    comment_cols=comment_cols,
                    channel_id=None,
                    n_recent=n_recent,
                    min_likes=min_likes,
            ...
            video_links_commented += [
                comment["video_link"] for comment in commented_comments[None]
            ]
        else:
            for channel_id in channel_ids:
                commented_comments[channel_id] = list(
                    self.db.get_comments(
                        comment_cols=comment_cols,
                ...
                video_links_commented += [
                    comment["video_link"] for comment in commented_comments[channel_id]
                ]
    ...
    def load_template_comments(self):
        if self.comments_conf is None:
            raise YoutubeManagerError(
                "Tried to load template comments "
        ...
        if self.comments_src == "dropbox":
            ...
            for file in self.dbox.ls(self.comments_conf["dropbox_folder_name"]).keys():
                if file[-4:] == ".txt":
                    self.dbox.download_file(
                        f'{self.comments_conf["dropbox_folder_name"]}/{file}',
        ...
        if self.comments_src in ("local", "dropbox"):
            comments_path = os.path.join(
                self.base_path,
                "../..",
                self.comments_conf["local_folder_name"],
                "*.txt",
            )
    ...
    def get_next_template_comment(
        ...
        if channel_id in self.template_comments:
            if self_comments_flags == 1:
                ...
                available_comments = (
                    self.template_comments[channel_id] + available_comments
                )
        ...
        if new_comments:  # If we have new template comments
            ...
            comment = [
                k
                for k, v in sorted(
                    comment_dates.items(), key=lambda p: p[1], reverse=False
                )
            ][0]
    ...
    def load_keys_from_cloud(self):
        if self.dbox is None:
            raise YoutubeManagerError(
                "`load_keys_from_cloud` was set to True "
    ...
    def simulate_uploads(self, channels: List, max_posted_hours: int = 2) -> Dict:
        ...
        for video_ind in range(num_videos):
            ...
            vid_title = "".join(
                random.choices(string.ascii_lowercase + " ", k=title_length)
            ).title()
            ...
            published_at = (
                datetime.utcnow() - timedelta(seconds=secs, minutes=mins, hours=hours)
            ).isoformat()
            upload = {
                "id": vid_id,
