...
class AbstractYoutubeApi(ABC):
    __slots__ = ('channel_name', 'channel_id', '_apis',
                 '_apis_errored', 'tag', 'playlist_ids')
    ...
    def __init__(self, config: Dict, tag: str, base_path: str) -> None:
        ...
        for cr_ind, creds in enumerate(config['credentials']):
            ...
            if 'apps.googleusercontent.com' not in creds['client_id']:
                continue
            self._apis.append(self._build_api(
        ...
        try:
            ...
            logger.error(
                f'Error while getting channel name and id: {str(e)}')
...
class YoutubeApiV3(AbstractYoutubeApi):
    ...
    def _build_api(client_id: str, client_secret: str, api_version: str, read_only_scope: str,
                   tag: str, base_path: str) -> googleapiclient.discovery.Resource:
        """
        ...
        while not ok and errors <= 5:
            try:
                ...
                if 'SERVICE_UNAVAILABLE' in str(e):
                    logger.warn(
                        "YT Service unavailable trying to get the username. Waiting 60 seconds and retrying..")
                    errors += 1
                    time.sleep(60)
                    if errors > 5:
                    ...
                    error_txt = f"Unknown Exception getting the username. Exiting..\n{e}"
    ...
    def comment(self, video_id: str, comment_text: str) -> None:
        ...
        try:
            ...
            response = self._comment_threads_insert(properties=properties,
                                                    part='snippet')

        except Exception as e:
            self._raise_if_catchable(e)
            if 'The comment thread could not be created due to insufficient permissions' in str(e):
                logger.warn(
                    f"Video id: {video_id} is probably private or a LiveStream, skipping..")
            else:
    ...
    def search_uploads(self, channels: str, max_posted_hours: int = 2) -> Dict:
        ...
        time_min_fmt = str(time_min.isoformat()).replace('+00:00', 'Z').replace(' ', 'T')
        search_response = self._apis[0].search().list(
    ...
    def get_video_comments(self, url: str = None, search_terms: str = None, comment_id: str = None) -> List:
        ...
        if comment_id is not None:
            ...
            raise Exception(
                "You should set either the url or the comment id to get the comment data.")
    ...
    def _get_video_comments_by_id(self, comment_id: str):
        ...
        comment_threads_response = self._apis[0].commentThreads().list(
            part="snippet",
            maxResults=100,
            id="{}{}".format(comment_id, page_token)
        ).execute()

        comments = []
        for comment_thread in comment_threads_response['items']:
            try:
                video_id = comment_thread['snippet']['topLevelComment']['snippet']['videoId']
                current_comment = {"url": f"https://youtube.com/watch?v={video_id}",
                                    "video_id": video_id,
                ...
                self._raise_if_catchable(e)
                logger.error(
                    f"Exception in get_video_comments() for {comment_thread}.")
                logger.error(f"{e}")
        ...
        while playlist_items_request:
            ...
            for playlist_item in playlist_items_response["items"]:
                ...
                if published_at >= (datetime.utcnow() - timedelta(hours=max_posted_hours)).replace(
                        tzinfo=timezone.utc) and "#shorts" not in playlist_item["snippet"]["title"].lower():
    ...
    def _comment_threads_insert(self, properties: Dict, **kwargs: Any) -> Dict:
        ...
        response = self._comment_api.commentThreads().insert(
